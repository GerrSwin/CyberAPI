...
import { NButton, NCard, NDropdown, NEllipsis, NGi, NGrid, NH3, NIcon, NInput, NText, useDialog, useMessage } from 'naive-ui'
...
// Filter and sort
function filterAndSort(apiCollections: APICollection[], keyword: string, sortOrder: string): APICollection[] {
  const collections = apiCollections.filter((item) => {
    if (!keyword) {
      return true
    }
    const arr: string[] = [item.name.toLowerCase(), item.description.toLowerCase()]
...
export default defineComponent({
  ...
  setup() {
    ...
    const keyword = ref('')
    const sortOrder = ref(settingStore.collectionSortType || SortType.LastModified)
  ...
  render() {
    const { sortOrder, createCollection, fetching, apiCollections, handleCollection, keyword } = this
    const sortOptions = [
      {
      ...
      <NGrid>
        ...
        <NGi span={16}>
          <NGrid xGap={10}>
            ...
            <NGi span={3}>
              <NDropdown
                class={s.sortOptionsClass}
                value={sortOrder}
                options={sortOptions}
                onSelect={this.changeSortType}
                renderLabel={(option) => {
                  return <span class={s.option}>{option.label}</span>
                }}
              >
    ...
    if (!fetching) {
      ...
      const arr = items.map((item) => {
        ...
        return (
          <NGi key={item.id}>
            <div
              class={s.collectionContentClass}
              onClick={() => {
            ...
            >
              <NCard title={item.name} v-slots={slots} hoverable>
                <div class={s.collecitonDescriptionClass}>
      ...
      collections = (
        <NGrid cols={'2 s:3 m:4 l:5 xl:6 2xl:7'} responsive={'screen'} xGap={20} yGap={20}>
          {arr}
        </NGrid>
      )
    }
