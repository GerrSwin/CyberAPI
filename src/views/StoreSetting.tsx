...
import { isWebMode, reload, showError } from '../helpers/util'
import s from './StoreSetting.module.css'

import { clearAppSetting } from '../commands/settings'
...
export default defineComponent({
  ...
  setup() {
    ...
    const clear = async (index: number) => {
      try {
        ...
        if (!isWebMode() && id === StoreKey.setting) {
          await clearAppSetting()
        } else {
          await clearStore(id)
        }
  ...
  render() {
    ...
    return (
      <NCard title={i18nStore('title')} class={s.storeClass}>
