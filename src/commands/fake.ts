...
export async function fakeDeleteAPICollection<T extends WithID>(storeName: string, id: string) {
  // Temporarily delete collection in a simple way
  const result = await fakeList<T>(storeName)
  let found = -1
  result.forEach((item, index) => {
    if (item.id == id) {
...
export async function fakeDeleteItems<T extends WithID>(storeName: string, ids: string[]) {
  const result = await fakeList<T>(storeName)
