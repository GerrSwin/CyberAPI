...
function convertPostManAPISetting(item: PostManSetting, collection: string) {
  ...
  if (uri && uri.includes('?')) {
    const arr = uri.split('?')
    uri = arr[0]
    // Strip the leading host+path part
    arr.shift()
...
function convertSwaggerSetting(params: {
  ...
  basePathENV.value = `${get(json, 'schemes.0')}://${get(json, 'host')}${get(json, 'basePath')}`
    ...
    forEach(value, (data, method) => {
      ...
      forEach(get(data, 'parameters'), (param) => {
        if (param.in === 'query') {
          ...
          forEach(get(json, defineKey.replace(/\//g, '.') + '.properties'), (value, key) => {
...
function convertPostManSetting(params: { result: ImportData; items: PostManSetting[]; collection: string; parentChildren: string[] }) {
...
function convertInsomniaSetting(params: { result: ImportData; items: InsomniaSetting[]; collection: string }) {
...
export async function importAPI(params: { category: ImportCategory; collection: string; fileData: string }): Promise<string[]> {
    ...
    case ImportCategory.Insomnia:
      {
        ...
        arr = arr.filter((item) => ['request', 'request_group'].includes(item._type))
