...
import { APICollection, createAPICollection, deleteAPICollection, listAPICollection, updateAPICollection } from '../commands/api_collection'
...
async function toggleFolderExpanded(collection: string, folder: string, expanded: boolean) {
...
export const useAPICollectionStore = defineStore('apiCollections', {
  ...
  actions: {
    ...
    async fetchExpandedFolders(collection: string) {
      const items = await getExpandedSettingStore().getItem<string[]>(collection)
      if (items) {
        this.expandedFolders = items
      }
    },
    ...
    async fetchActiveTabs() {
      const data = await getTabActiveStore().getItem<TabActiveData>(tabActiveKey)
    ...
    async updateActiveTab(params: { id: string; activeTab: string }) {
      ...
      await getTabActiveStore().setItem(tabActiveKey, Object.assign({}, this.activeTabs))
    },
    ...
    async openFolder(collection: string, folder: string) {
      // localforage is fast, so don't record in-progress state
