...
export const useAPIFolderStore = defineStore('apiFolders', {
  ...
  actions: {
    ...
    async addChild(params: {
      // Folder id (empty means add to top level)
      id: string
      // Items to add
      children: string[]
      // Insert before which element
      before?: string
    }) {
      if (this.updating) {
        return
      ...
      const addChild = (children: string, child: string, beforeItem: string) => {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        // Check whether this child already has a folder
        ...
        if (currentFolder) {
          currentFolder.children = addChild(currentFolder.children, child, params.before || '')
    ...
    async remove(id: string): Promise<void> {
      ...
      try {
        ...
        this.apiFolders = this.apiFolders.filter((item) => !folderIds.includes(item.id))
