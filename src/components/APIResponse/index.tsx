...
import { getLatestResponse, getResponseBody, HTTPResponse, HTTPStats, ResponseBodyResult } from '../../commands/http_response'
import { createEditor, foldAll, replaceContent, unfoldAll } from '../../helpers/editor'
import { convertHTTPHeaderName, formatLatency, showError, writeFileToDownload, writeTextToClipboard } from '../../helpers/util'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleToCURL = async () => {
      ...
      try {
        // Cookies keep updating, so fetch again now
        await cookieStore.fetch()
        // The req object has not applied the uri part; adjust it first
        apiSettingStore.fillValues(req)
        const value = await convertRequestToCURL(collection, req, cookieStore.cookies)
  ...
  render() {
    ...
    const headerSlots = {
      trigger: () => <span class={s.header}>H</span>,
    }
    ...
    const cookieSlots = {
      trigger: () => (
        <NIcon class={[s.info, s.cookie]} size={17}>
          <BowlingBallOutline />
        </NIcon>
      ),
    }
    ...
    const headerDescriptionItems: VNode[] = []
    const cookieDescriptionItems: VNode[] = []
      ...
      headers.forEach((values, key) => {
        values.forEach((value, index) => {
          ...
          headerDescriptionItems.push(
            <NDescriptionsItem label={convertHTTPHeaderName(key)} key={`${key}-${index}`} span={3}>
              {value}
            </NDescriptionsItem>,
          )
        })
    ...
    const previewWrapperCls = {
      [s.previewWrapper]: true,
      [s.isDark]: false,
    }
    if (this.isDark) {
      previewWrapperCls[s.isDark] = true
    }
    ...
    return (
      <div class={s.responseClass}>
        {apiID && (
          <div class={s.responseList}>
            {' '}
            <APIResponseList id={apiID} collapseAll={handleCollapseAll} expandAll={handleExpandAll} foldDisabled={previewMode} />{' '}
          </div>
        )}
        <NSpace class={s.infos}>
          {apiID && (
          ...
          {cookieDescriptionItems.length !== 0 && (
            <NPopover v-slots={cookieSlots} trigger="click" placement="bottom" contentStyle={popupContentStyle}>
              <NDescriptions labelPlacement="left" size="small">
          ...
          {headerDescriptionItems.length !== 0 && (
            <NPopover v-slots={headerSlots} trigger="click" placement="bottom">
              <NDescriptions contentStyle={popupContentStyle} labelPlacement="left" size="small">
